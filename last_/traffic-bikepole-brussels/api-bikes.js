...
async function getPoles() {
  ...
  return fetch(API_URL).then((/** @type {{ json: () => any; }} */ response) => response.json()).then((/** @type {{ features: any; }} */ json) => { return json.features; });
}
...
export async function getParsedPoles() {
  ...
  return allPoles.map((/** @type {{ properties: { device_name: any; road_nl: any; road_fr: any; road_en: any; }; }} */ el) => {
    // @ts-ignore
    return {
...
export async function fetchLiveCounts() {
  ...
  return await fetch(API_URL).then((/** @type {{ json: () => any; }} */ response) => response.json()).then((/** @type {{ data: any; }} */ json) => json.data);
}
 ...
 */
async function fetchHistory(poleName, startDate, endDate) {
  ...
  return fetch(API_URL).then((/** @type {{ json: () => any; }} */ response) => response.json());
}

/**
 *history for ONE pole
 * @param {String} poleName
...
export async function fetchParsedHistory(poleName, startDate, endDate, groupByDay = false) {
  ...
  const poleHistoryParsed = poleHistory.data.map((/** @type {{ time_gap: any; count_date: string; count: any; }} */ historyItem) => {
    const timeGapForEl = poleHistory.timeGaps.find((/** @type {any[]} */ timeGap) => timeGap[0] === historyItem.time_gap); // TimeGap is an array with : index, startDate, endDate
  ...
  poleHistoryParsed.forEach((/** @type {{ hour: any; date: any; count: any; }} */ historyItem) => {
  ...
  if (groupByDay) {
    ...
    poleHistoryParsed.forEach((/** @type {{ date: any; count: any; }} */ historyItem) => {
      let indexOfDay = 0;
