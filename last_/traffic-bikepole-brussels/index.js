...
const main = async () => {
  ...
  allPolesBike.forEach(async (/** @type {{ name: string; }} */ pole) => {
    ...
    fetchParsedHistory(pole.name, POLES_HISTORY_START_DATE, POLE_HISTORY_END_DATE, true)
      .then((history) => {
        ...
        const poleIndex = allPolesBike.findIndex((/** @type {{ name: any; }} */ el) => el.name === pole.name);
  ...
  setInterval(async () => {
    ...
    allPolesBike = newPoles.map((/** @type {{ name: any; address: any; }} */ newEl) => {
      //se ho già un pole che si chiama cosi', prendi il vecchio pole e sostituiscilo con il nuovo:
      const previousDataForThisPole = allPolesBike.find((/** @type {{ name: any; }} */ pastEl) => pastEl.name === newEl.name);
      return {
        name: newEl.name,
    ...
    allTraversesCar = newTraverses.map((/** @type {{ name: any; location: any; }} */ newEL) => {
      const carExist = allTraversesCar.find((/** @type {{ name: any; }} */ pastEl) => pastEl.name === newEL.name);
      return {
        name: newEL.name,
  ...
  setInterval(async () => {
    ...
    allPolesBike = allPolesBike.map((/** @type {{ name: string | number; history: any[]; }} */ pole) => {
      ...
      const poleHistoryWithoutThisHour = pole.history.filter((/** @type {{ hour: number; }} */ el) => el.hour !== parseInt(count.cnt_time.split(" ")[1].substring(0, 2)));
    ...
    allTraversesCar = allTraversesCar.map((/** @type {{ name: string | number; history: any[]; }} */ traverse) => {
      ...
      if (count.count === null || count.start_time === '-') {
        ...
        */
        ...
        const pastHistoryCars = traverse.history.filter((/** @type {{ hour: number; }} */ el) => el.hour !== parseInt(count.start_time.split(" ")[1].substring(0, 2)));
        return {
          ...traverse,
          history: [
            ...pastHistoryCars,
    ...
    const totalCountBikesToday = allPolesBike.map((/** @type {{ history: any; }} */ pole) => pole.history).flat().filter((/** @type {{ date: string; }} */ history) => history.date === dateToYYYYMMDD(new Date())).map((/** @type {{ count: any; }} */ e) => e.count).reduce((/** @type {any} */ prec, /** @type {any} */ post) => prec + post);
    const totalCountCarsToday = allTraversesCar.map((/** @type {{ history: any; }} */ traverse) => traverse.history).flat().filter((/** @type {{ date: string; }} */ history) => history.date === dateToYYYYMMDD(new Date())).map((/** @type {{ count: any; }} */ e) => e.count).reduce((/** @type {any} */ prec, /** @type {any} */ post) => prec + post);
